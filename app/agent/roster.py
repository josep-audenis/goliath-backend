...
ROSTER: dict[str, tuple[str, str]] = {
    "partner": ("Open and close the briefing; frame the thesis and deliver the recommendation.", "George"),
    ...
    "risk_analyst": ("Surface risks, weak signals, and calibrate confidence.", "Laura"),
}

# The partner is the presenter — always present, added by the backend (not the
# planner). It frames the experts and delivers the closing recommendation.
PARTNER_ROLE = "partner"

DEFAULT_ROLES = ["market_mapper", "company_scout", "current_opportunities", "risk_analyst"]


def partner_agent() -> AgentPlan:
    return make_agent_plan(PARTNER_ROLE, 0)
